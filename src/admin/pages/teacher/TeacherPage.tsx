...
const TeacherPage: React.FC = () => {
    ...
    const { isLoading, isSuccess, data } = useQuery<UserInfoContextType, AxiosError, any>(
        ...
        async () => {
            const res = await apiClient.get(`/api/staff/get-teacher/${id}`);
            return res.data;
        },
        {
            onError: (error) => {
    ...
    async function deleteGP(selected: string[], params: { allChecked: boolean; userSearch?: string }) {
        try {
            ...
            const { data } = await apiClient.delete('/api/good-points/delete-good-point', {
                data: {
                    all: allChecked,
                    selected,
                    studentId: id,
    ...
    async function resetPassword() {
        try {
            const res = await apiClient.put(`/api/staff/reset-password-by-admin/${id}`);
