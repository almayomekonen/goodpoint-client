...
export function useAddSchool() {
    ...
    return useMutation(
        async (data: { name: string; code: string }) => {
            const postReq = await apiClient.post('/api/schools/add-school', data);
            return postReq.data;
        },
        {
            onSuccess: (result) => {
...
export function useEditSchool() {
    ...
    return useMutation(
        async (data: { name: string; code: string; id: number }) => {
            const postReq = await apiClient.patch('/api/schools/update-school', data);
            return postReq.data;
        },
        {
...
export function useDeleteSchool() {
    ...
    return useMutation(
        async (id: number) => {
            const postReq = await apiClient.delete('/api/schools/delete-school', { data: { id } });
            return postReq.data;
        },
        {
            onSuccess: (schoolId) => {
...
export function useAddAdmin(schoolId: number) {
    ...
    return useMutation(async (admin: Admin) => {
        const postReq = await apiClient.post('/api/staff/add-admin', { ...admin, schoolId });
        return postReq.data;
    });
}
...
export function useDeleteAdmin(schoolId: number) {
    ...
    return useMutation(
        async (data: { adminId: string; schoolId: number }) => {
            const postReq = await apiClient.delete('/api/staff/delete-admin', { data });
            return postReq.data;
        },
        {
