...
const _ExportReport = () => {
    ...
    const { isLoading, data: allSchoolClasses = [] } = useQuery<ClassList[]>(['populated-classes'], async () => {
        return (await apiClient.get<ClassList[]>('/api/classes/get-populated-school-classes')).data;
    });

    const { data: allClassStudents } = useQuery<StudentsByClass[]>(
        ['classStudents', values.grade],
        async () => (await apiClient.get<StudentsByClass[]>(`/api/student/get-students-report/${values.grade}`)).data,
